<%-- 
    Document   : index
    Created on : Jul 15, 2021, 2:15:10 PM
    Author     : Josema
--%>
...
<head>
    ...
    <link rel="stylesheet" href="css/cafe_guayoyo.css">
    <link rel="icon" href="assets/logo_guayoyo_cafe.png" type="image/png">
    <title>Guayoyo Café - tu portal al mejor café</title>
...
<body>
    <header>
        <div class="banner">
            <video width="" height="" autoplay muted loop playsinline >
                <source src="assets/cafe_video.mp4" type="video/mp4">
            ...
            <div class="banner-content">
                ...
                <div class="empleado">
                    <a href="<%= ( request.getSession().getAttribute("isLoggedIn").equals(false) ? "#ingreso-empleado\" onclick=\"showLoginForm(this);\"" : request.getContextPath()+"/control-panel\"") %>><%= ( request.getSession().getAttribute("isLoggedIn").equals(false) ? "Ingreso Empleados" : "Panel de Control") %></a>
                </div>
                ...
                <div class="quick-actions">
                    <p>Nos especializamos en la venta y distribución de café en granos de origen local e importado.</p>
                    <a href="#seleccion">Explora nuestra selección</a>
                </div>

                <div class="links">
                    Para compras
                    <section class="link">
                        <img src="assets/mercado-libre.png" alt="logo mercadolibre" height="50" width="50">
                        <p>Mercadolibre</p>
                    </section>
                </div>
            </div>
    ...
    <main>
        <div id="seleccion">
            <h2>Nuestros café</h2>
            <%= (request.getSession().getAttribute("isLoggedIn").equals(false) ? "" : "<a href=\"control-panel\">Agregar un nuevo cafe</a>" ) %>
            <div class="galeria">
                <%  ArrayList<Cafe> cafes = (ArrayList) request.getAttribute("cafes");
                    for (int i = 0; i < cafes.size(); i++) { %>
                    <div class="cafe">
                        <img src="<%= cafes.get(i).getImg()%>" alt="<%= cafes.get(i).getNombre() %> imagen">
                        <h4><%= cafes.get(i).getNombre() %></h4>
                        <p><%= (cafes.get(i).getDescripcion().length() > 80 ? cafes.get(i).getDescripcion().substring(0, 80) + "..." : cafes.get(i).getDescripcion()) %></p>
                        <a href="<%= request.getContextPath() %>/cafe?i=<%= cafes.get(i).getID() %>">Leer más</a>
                    </div>     
                <% } %>
    ...
    <div class="modal" id="ingreso-modal">
        <div class="modal-content">
            <div class="modal-form">
                <img src="assets/logo_guayoyo_cafe.png" alt="logo guayoyo cafe" class="logo">
                <h2>Ingreso Empleado</h2>
                <p>Acceso restringuido!</p>
                <p>Si no es empleado de Guayoyo Cafe por favor cierre esta ventana.</p>
